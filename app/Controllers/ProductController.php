    ...
    public function show(): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_comment'])) {
            ...
            if (!empty($customerName) && !empty($commentText)) {
                ...
                foreach ($positiveWords as $word) {
                    if (mb_strpos($lowerComment, $word) !== false) $positiveScore++;
                }
                foreach ($negativeWords as $word) {
                    if (mb_strpos($lowerComment, $word) !== false) $negativeScore++;
                }
                foreach ($inquiryWords as $word) {
                    if (mb_strpos($lowerComment, $word) !== false) $inquiryScore++;
                }
                $positiveReplies = [
                    "أهلاً يا {$customerName}، الشكر موصول لثقتك في متجرنا، ونتمنى دائماً تقديم الأفضل! 😊",
                    "هذا التقييم يسعد إدارة المتجر جداً يا {$customerName}، شرفنا التعامل معك ونتمنى لك تجربة استخدام موفقة.",
                    "يا مرحباً بك، كلمات الإشادة نعتز بها، شكراً لاختيارك تكنو ستور.",
                    "تسلم يا {$customerName} على هذا الرأي الجميل، رضاك هو هدفنا الأول."
                ];
                $negativeReplies = [
                    "أهلاً يا {$customerName}، نعتذر بشدة عن هذه التجربة غير المرضية، سيتم التواصل معك من الدعم الفني فوراً لحل المشكلة.",
                ...
                $inquiryReplies = [
                    "أهلاً يا {$customerName}، تم تحويل استفسارك لقسم المبيعات وسيتم الرد عليك بالتفاصيل حالا.",
                ...
                $neutralReplies = [
                    "أهلاً يا {$customerName}، تم تسجيل تعليقك بنجاح وسيتم مراجعته من الإدارة.",
                    "مرحباً بك، تفاعلك في تكنو ستور محل تقدير كبير لدينا.",
                    "شكراً لمرورك وتعليقك، يسعدنا تواصلك الدائم."
                ];
                if ($negativeScore > 0) {
                    $aiReply = $negativeReplies[array_rand($negativeReplies)];
                } elseif ($inquiryScore > 0) {
                    $aiReply = $inquiryReplies[array_rand($inquiryReplies)];
                } elseif ($positiveScore > 0) {
                    $aiReply = $positiveReplies[array_rand($positiveReplies)];
                } else {
                    $aiReply = $neutralReplies[array_rand($neutralReplies)];
                }
                $commentModel = new Comment();
                $commentModel->createComment((int)$id, $customerName, $commentText, $aiReply);
            }
        ...
        for ($i = 1; $i <= 5; $i++) {
            if ($rating >= $i) {
                ...
                $starsHtml .= '<i class="fa-regular fa-star-half-stroke fa-flip-horizontal"></i>';
            } else {
                $starsHtml .= '<i class="fa-regular fa-star"></i>';
            }
        }
        $priceParts = explode('.', number_format($product['price'], 2, '.', ''));
